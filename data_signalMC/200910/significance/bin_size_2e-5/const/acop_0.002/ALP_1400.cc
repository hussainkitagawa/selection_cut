...
void ALP_1400(){
	...
	// obtain tree
        TTree* tree	 = (TTree*)file->Get("alp_1400GeV");

	// Set output file
        TFile* outputFile = new TFile("significance_data.root","recreate");
        TTree* outputTree = new TTree("alp_1400GeV", "data_MC");
...
/****************************************************************/
	...
	double optimized_data_events;
		...
		data_events = 90 * MC_ibin;//background is constant according to Invariant Mass>=1400GeV
