...
void ALP_200(){
	...
	// obtain tree
        TTree* tree	 = (TTree*)file->Get("alp_200GeV");

	// Set output file
        TFile* outputFile = new TFile("significance_data.root","recreate");
        TTree* outputTree = new TTree("alp_200GeV", "data_MC");
...
/****************************************************************/
	...
	GeneratorMC_Acop->Sumw2();// Sumw2 has to be done before Scale()
        ...
        cout<< "Expected MC events(200GeV);     " << expected_MC << '\n';
	...
	double optimized_data_events;
		...
		data_events = 90 * MC_ibin;//background is constant according to Invariant Mass>=200GeV
