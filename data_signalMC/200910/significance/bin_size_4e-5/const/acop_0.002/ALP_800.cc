...
void ALP_800(){
	...
	// obtain tree
        TTree* tree	 = (TTree*)file->Get("alp_800GeV");

	// Set output file
        TFile* outputFile = new TFile("significance_data.root", "recreate");
        TTree* outputTree = new TTree("alp_800GeV", "data_MC");
...
/****************************************************************/
	...
	tree->SetBranchAddress("800GeV_Energy_ALP", 	&Energy_ALP);
	tree->SetBranchAddress("800GeV_Energy_Photon1", &Energy_Photon1);
	tree->SetBranchAddress("800GeV_Energy_Photon2", &Energy_Photon2);
	tree->SetBranchAddress("800GeV_Mass_ALP", 	&Mass_ALP);
	tree->SetBranchAddress("800GeV_Mass_Photon1", 	&Mass_Photon1);
	tree->SetBranchAddress("800GeV_Mass_Photon2", 	&Mass_Photon2);
	tree->SetBranchAddress("800GeV_Theta_Photon1", 	&Theta_Photon1);
	tree->SetBranchAddress("800GeV_Theta_Photon2", 	&Theta_Photon2);
	tree->SetBranchAddress("800GeV_Eta_Photon1", 	&Eta_Photon1);
	tree->SetBranchAddress("800GeV_Eta_Photon2", 	&Eta_Photon2);
	tree->SetBranchAddress("800GeV_TransverseMomentum_Photon1", 	&TransverseMomentum_Photon1);
	tree->SetBranchAddress("800GeV_TransverseMomentum_Photon2", 	&TransverseMomentum_Photon2);
	tree->SetBranchAddress("800GeV_Acoplanarity", 	&DiphotonAcoplanarity);
		



	//histogram of Acoplanarity  
        //TH1D* Generator_Acop_800GeV= new TH1D("", "", 540, 0, 0.0216);//binning size is 4e-5
       ...
       /**********   Start main Loop of Generator level with 800 GeV ALP mass(ALP_GeneratorMC.root)   **********/
...
/****************************************************************/
	...
	GeneratorMC_Acop->Sumw2();// Sumw2 has to be done before Scale()
        ...
        cout<< "Expected MC events(800GeV);     " << expected_MC << '\n';
...
//Set parameters for Loop
        ...
        //histogram of TGraph :: Calculate significance for each ALP mass for 800GeV 
	...
	double optimized_data_events;
		...
		data_events = 200 * MC_ibin;//background is constant according to Invariant Mass>=800GeV
