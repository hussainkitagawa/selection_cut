...
*/  	diphoton->SetBranchAddress("PhotonPt1",			&MC_PhotonPt1);
	...
	/*    histogram of Acoplanarity    */
      	TH1D *MC_Acop = new TH1D("", "", 500, 0, 0.021);//0.021
...
/****************************************************************/
	...
	/*    histogram of Acoplanarity    */
//     	TH1D *data_Acop = new TH1D("", "", 1, 0, 0.021);
...
/****************************************************************/
	...
	MC_Acop->Sumw2();
	long double A1 = data_Acop->Integral() / MC_Acop->Integral();
	long double A2 = 32 * 7363 * 6.0821/10000; // expected detected signal events of ALP in a data (30 inverse femto barn)(!= not signal of MC)
	cout << " A1 is " << A1 <<'\n';
	long double A3 = diphoton->GetEntries();
	cout << " A2 is " << A2 <<'\n';
	cout << " A2/A3 is " << A2/A3 <<'\n';
    	MC_Acop->Scale(A2/A3);
	MC_Acop->SetLineColor(kRed);
	data_Acop->SetLineColor(kBlue);
    	data_Acop->Draw("hist e1"); 
