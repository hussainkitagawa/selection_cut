...
/****************************************************************/
	...
	/*    histogram of Acoplanarity    */
      	TH1D *data_Acop = new TH1D("", "", 54, 0, 0.00216);
...
/****************************************************************/
	...
	double A4 = MC_Acop_all->Integral(0, 50);//maybe 142 events
	cout << " A4 = " << A4 <<'\n';
...
/*	double end_DiphotonAcop = 0.00204;
	double start_DiphotonAcop = 0.00216/54;
	...
	while (start_DiphotonAcop <= end_DiphotonAcop) {
		...
		//Loop for signalMC
		MC_events = MC_Acop->Integral(0, MC_nbins);			
